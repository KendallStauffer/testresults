...
def pin_gather_attrs() -> str:
    return (
        f'action="{BASE_URL}/gather_pin" method="POST" '
        f'input="speech" timeout="6" speechTimeout="{PIN_SPEECH_TIMEOUT}" '
        f'language="{TTS_LANGUAGE}" transcriptionEngine="{ASR_ENGINE}" '
        f'hints="{escape_xml(PIN_HINTS)}"'
    )


def menu_gather_attrs(action_path: str, hints: str) -> str:
    return (
        f'action="{BASE_URL}{action_path}" method="POST" '
        f'input="dtmf speech" numDigits="1" timeout="5" '
        f'speechTimeout="{MENU_SPEECH_TIMEOUT}" language="{TTS_LANGUAGE}" '
        f'transcriptionEngine="{ASR_ENGINE}" hints="{escape_xml(hints)}"'
    )
...
def voice():
  ...
  <Gather {pin_gather_attrs()}>
    {say("Say your six digit PIN now.")}
  </Gather>
...
def pin_retry_xml():
  ...
  <Pause length="1"/>
  <Gather {pin_gather_attrs()}>
    {say("Sorry, I did not get that. Say your six digit PIN now, one number at a time.")}
  </Gather>
