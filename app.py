...
def voice():
    ...
    response.add(plivoxml.SpeakElement("No input received. Goodbye.", voice="Polly.Joanna", language="en-US"))
...
def gather_pin():
    ...
    logger.info(f"GATHER_PIN called | Digits = '{digits}' | CallUUID = {call_uuid}")
    ...
    if len(digits) == 6:
        ...
        response.add(plivoxml.SpeakElement(f"Thank you. You entered {speak_pin_digits(digits)}.", voice="Polly.Joanna", language="en-US"))
    else:
        log_call("PIN_INVALID", {"digits": digits})
        response.add(plivoxml.SpeakElement("Sorry, please enter 6 digits.", voice="Polly.Joanna", language="en-US"))
