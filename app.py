...
def voice():
    ...
    gather = Gather(
        ...
        speech_timeout="auto",
        language="en-US",
        speech_model="google"          # ← Google recognizer (better for numbers)
    )
...
def confirm_pin():
    ...
    # === Read results immediately after confirmation ===
    resp.pause(length=0.4)
    resp.say("Thank you. Here are your milk test results.", voice="Polly.Joanna", language="en-US")

    # Get the last entered PIN (we'll store it properly in next step if needed)
    # For now we ask one final time - but very quickly
    gather = Gather(
        action="/final_read",
        num_digits=6,
        timeout=8,
